...
class ModelsEmailUnico{
    ...
    public function valEmailUnico($email, $editarUnico = null, $id = null)
    {
        ...
        if(!empty($this->editarUnico) AND ($this->editarUnico == true)){
            $valEmailUnico->exeReadEspecifico("SELECT idPessoa FROM pessoa WHERE email =:email AND idPessoa <>:id LIMIT :limit", "email={$this->email}&limit=1&id={$this->id}");
        }else{
            $valEmailUnico->exeReadEspecifico("SELECT idPessoa FROM pessoa WHERE email =:email LIMIT :limit", "email={$this->email}&limit=1");
        }        
