...
class ModelsValUsuario{
    ...
    public function valUsuario($usuario, $editarUnico = null, $id = null)
    {
        ...
        if(!empty($this->editarUnico) AND ($this->editarUnico == true)){
            $valUsuario->exeReadEspecifico("SELECT idPessoa FROM pessoa WHERE nome =:usuario AND idPessoa <>:id LIMIT :limit", "usuario={$this->usuario}&limit=1&id={$this->id}");
        }else{
            $valUsuario->exeReadEspecifico("SELECT idPessoa FROM pessoa WHERE nome =:usuario LIMIT :limit", "usuario={$this->usuario}&limit=1");
        }        
